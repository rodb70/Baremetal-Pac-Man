...
static Sprite array[ 4 ];

extern volatile unsigned char *frame_buffer_init( void );
extern int frame_buffer_switch(int offset);
...
NEW_LIFE:
    ...
    uint64_t game_plan[ 3 ][ 7 ] =
    {
        { 7000000, 20000000, 7000000, 20000000, 5000000, 20000000, 5000000 },
    ...
    while( 1 )
    {
        ...
        if( game_plan[ game_pace ][ game_tick ] < timer_get_relative( game_timer ) &&
                game_tick != 7 && !frightened_mode )
        {
            timer_start(&game_timer);
            game_tick++;
            if( !( game_tick % 2 ))
        ...
        if(( timer_get_relative( fruit_timer ) > 10000000 && game_ptr->fruit_set) ||
                game_ptr->fruit_set > 60 )
        {
            /*remove fruit*/
        ...
        offset = frame_buffer_switch( offset );
